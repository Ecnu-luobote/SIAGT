...
@register('sr-scale-trans')
class SRtrans(Dataset):
    ...
    def __getitem__(self, idx):
        img_lr, img_attack = self.dataset[idx]
        hr = torch.ones(round(img_lr.shape[-2]*self.scale),round(img_lr.shape[-1]*self.scale)) # assume int scale
        hr_coord = make_coord(hr.shape[-2:], flatten=True)
        cell = torch.ones_like(hr_coord)
        cell[:, 0] *= 2 / hr.shape[-2]
        cell[:, 1] *= 2 / hr.shape[-1]
